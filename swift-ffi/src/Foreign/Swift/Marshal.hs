...
-- | The FFIResult is used to wrap the original type of the function and is used
-- to produce the type of the foreign exported function. ToMoatType converts
-- the Haskell foreign wrapper function type into a Swift type. Such as:
--
--  foreign export ccall something :: FFIResult (a -> IO b)
--
-- on the swift side we'll have the type generated by:
--
--  toMoatType (Proxy @(FFIResult (a -> IO b)))
--
