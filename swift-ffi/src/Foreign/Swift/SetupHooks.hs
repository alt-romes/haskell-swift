...
foreignSwiftSetupHooks libName = let xcfPath = buildDir </> libName <.> "xcframework" in
  ...
  <> noSetupHooks
    { configureHooks = noConfigureHooks
      { preConfComponentHook = Just setGhcOptions
      }
...
setGhcOptions :: PreConfComponentInputs -> IO PreConfComponentOutputs
setGhcOptions PreConfComponentInputs{..} = do
  let new_opts = emptyBuildInfo
        { {-ghc--}options = PerCompilerFlavor ["-fplugin Foreign.Swift.Lib"] []
        }
  return PreConfComponentOutputs
    { componentDiff = buildInfoComponentDiff (componentName component) new_opts }
...
postBuild libName PostBuildComponentInputs{..} = do
  ...
  -- Swift module with definitions common to the FFI modules
  let commonModPath = swiftSourcesDir </> "Foreign" </> "Haskell" </> "Common" <.> "swift"
  createDirectoryIfMissing True (takeDirectory commonModPath)
  B.writeFile commonModPath $ B.unlines $ map B.pack
      [ "// This file is automatically generated by swift-ffi"
      , "// Do not edit this file directly!"
      , ""
      , "import Foundation"
